...
// ─── Database ─────────────────────────────────────────────────────────────────
const db = new DatabaseSync(path.join(__dirname, 'appointments.db'));
...
db.exec(`CREATE TABLE IF NOT EXISTS vehicle_health (
...
// Mark users created before email verification feature as verified
try { db.exec("UPDATE users SET email_verified=1 WHERE verification_token IS NULL AND email_verified=0"); } catch {}
...
function buildSendGrid() {
  const key = getSetting('sg_api_key') || process.env.RESEND_API_KEY || '';
  if (key.trim()) {
...
function renderTemplate(type, vars) {
  ...
  let subject = getSetting(`tpl_${type}_subject`) || tpl.subject;
  let body    = getSetting(`tpl_${type}_body`)    || tpl.body;
  const friendly = {
    '[nombre del cliente]': vars.name,
    '[fecha]': vars.date,
    '[hora]': vars.time,
    '[vehículo]': vars.vehicle,
    '[tipo de aceite]': vars.oil_type,
    '[dirección]': vars.location,
    '[notas]': vars.notes,
    '[enlace de recuperación]': vars.resetLink,
    '[enlace de verificación]': vars.verifyLink,
  };
  const replace = s => {
    for (const [k, v] of Object.entries(friendly)) {
      if (v !== undefined && v !== null) s = s.split(k).join(v);
    }
    s = s.replace(/\{\{(\w+)\}\}/g, (_, k) => vars[k] !== undefined ? vars[k] : '');
    return s;
  };
...
async function sendEmail(to, type, vars) {
  ...
  const fromEmail = getSetting('sg_from_email') || process.env.RESEND_FROM || 'onboarding@resend.dev';
  const fromName  = getSetting('sg_from_name')  || 'Cambio de Aceite';
...
async function sendConfirmation(appt) {
  ...
  await sendEmail(appt.email, 'appointment_confirmed', {
    ...
    oil_type: appt.oil_type || '',
    location: appt.location ? `📍 ${appt.location}` : '',
    notes:    appt.notes    ? `📝 ${appt.notes}` : '',
    // Legacy vars for backward compat with saved HTML templates
...
app.post('/auth/register', async (req, res) => {
  ...
  const r = db.prepare('INSERT INTO users (name,email,phone,password_hash,verification_token,email_verified) VALUES (?,?,?,?,?,0)')
    .run(name.trim(), email.toLowerCase(), phone || '', hashPassword(password), vtoken);
...
app.post('/auth/login', (req, res) => {
  ...
  if (isTechLogin && user.role !== 'technician')
    return res.redirect('/tech-login?error=norole');
...
app.post('/api/me/resend-verification', requireCustomer, async (req, res) => {
  const user = db.prepare("SELECT id,name,email,email_verified FROM users WHERE id=?").get(req.session.customerId);
  if (!user || user.email_verified) return res.json({ success: true });
  const vtoken = crypto.randomBytes(32).toString('hex');
  db.prepare('UPDATE users SET verification_token=? WHERE id=?').run(vtoken, user.id);
  const verifyUrl = `${req.protocol}://${req.get('host')}/auth/verify-email?token=${vtoken}`;
...
app.post('/auth/resend-verification', async (req, res) => {
  ...
  const user = db.prepare("SELECT id, name FROM users WHERE email=? AND email_verified=0 AND role='customer'").get(email.toLowerCase());
  if (user) {
    const vtoken = crypto.randomBytes(32).toString('hex');
    db.prepare('UPDATE users SET verification_token=? WHERE id=?').run(vtoken, user.id);
    const verifyUrl = `${req.protocol}://${req.get('host')}/auth/verify-email?token=${vtoken}`;
...
app.get('/api/me', requireCustomer, (req, res) => {
  const user = db.prepare('SELECT id,name,email,phone,zip_code,address_street,address_number,address_city,email_verified FROM users WHERE id=?').get(req.session.customerId);
  res.json(user || {});
});

app.delete('/api/me', requireCustomer, (req, res) => {
  const uid = req.session.customerId;
  db.prepare("UPDATE appointments SET status='cancelled' WHERE user_id=? AND status='pending'").run(uid);
  db.prepare('DELETE FROM vehicles WHERE user_id=?').run(uid);
  db.prepare('DELETE FROM vehicle_health WHERE user_id=?').run(uid);
  db.prepare('DELETE FROM users WHERE id=?').run(uid);
  req.session.destroy(() => res.json({ success: true }));
});

app.put('/api/me/address', requireCustomer, (req, res) => {
  ...
  db.prepare('UPDATE users SET address_street=?,address_number=?,address_city=?,zip_code=? WHERE id=?')
    .run(address_street.trim(), (address_number||'').trim(), address_city.trim(), zip_code.trim(), req.session.customerId);
  res.json({ success: true });
});

app.put('/api/me/zip', requireCustomer, (req, res) => {
  const { zip_code } = req.body;
  if (!zip_code?.trim()) return res.status(400).json({ error: 'ZIP requerido' });
  db.prepare('UPDATE users SET zip_code=? WHERE id=?').run(zip_code.trim(), req.session.customerId);
  res.json({ success: true });
});

// Vehicles
app.get('/api/vehicles', requireCustomer, (req, res) => {
  res.json(db.prepare('SELECT * FROM vehicles WHERE user_id=? ORDER BY created_at ASC').all(req.session.customerId));
});

app.post('/api/vehicles', requireCustomer, (req, res) => {
  const { vehicle, oil_type, location, notes, color } = req.body;
if (!vehicle?.trim()) return res.status(400).json({ error: 'Vehículo requerido' });
  const r = db.prepare('INSERT INTO vehicles (user_id,vehicle,oil_type,location,notes,color) VALUES (?,?,?,?,?,?)')
    .run(req.session.customerId, vehicle.trim(), oil_type || 'Convencional', location || '', notes || '', color || '');
  res.json({ success: true, vehicle: db.prepare('SELECT * FROM vehicles WHERE id=?').get(r.lastInsertRowid) });
});

app.delete('/api/vehicles/:id', requireCustomer, (req, res) => {
  const v = db.prepare('SELECT id FROM vehicles WHERE id=? AND user_id=?').get(+req.params.id, req.session.customerId);
...
app.get('/api/vehicles/:id/history', requireCustomer, (req, res) => {
  const v = db.prepare('SELECT id FROM vehicles WHERE id=? AND user_id=?').get(+req.params.id, req.session.customerId);
  if (!v) return res.status(404).json({ error: 'No encontrado' });
  res.json(
    db.prepare("SELECT * FROM appointments WHERE vehicle_id=? AND status='completed' ORDER BY date DESC, time DESC")
      .all(+req.params.id)
  );
});

// Pending appointments per vehicle
app.get('/api/vehicles/:id/appointments', requireCustomer, (req, res) => {
  const v = db.prepare('SELECT id FROM vehicles WHERE id=? AND user_id=?').get(+req.params.id, req.session.customerId);
  if (!v) return res.status(404).json({ error: 'No encontrado' });
  const today = new Date().toISOString().split('T')[0];
  res.json(
    db.prepare("SELECT * FROM appointments WHERE vehicle_id=? AND date>=? AND status='pending' ORDER BY date,time")
      .all(+req.params.id, today)
  );
});

app.get('/api/my-appointments', requireCustomer, (req, res) => {
  const today = new Date().toISOString().split('T')[0];
  res.json(
    db.prepare("SELECT * FROM appointments WHERE user_id=? AND date>=? AND status='pending' ORDER BY date,time")
      .all(req.session.customerId, today)
  );
});

const TIME_SLOTS = ['8:00','9:00','10:00','11:00','12:00','13:00','14:00','15:00','16:00','17:00'];

app.get('/api/appointments/available-slots', requireCustomer, (req, res) => {
  const { date } = req.query;
  if (!date) return res.status(400).json({ error: 'Date required' });
  const taken = db.prepare("SELECT time FROM appointments WHERE date=? AND status='pending'").all(date).map(r => r.time);
  res.json({ available: TIME_SLOTS.filter(s => !taken.includes(s)), taken });
});

app.post('/api/appointments', requireCustomer, (req, res) => {
  const u = db.prepare('SELECT email_verified FROM users WHERE id=?').get(req.session.customerId);
  ...
  const exists = db.prepare("SELECT id FROM appointments WHERE date=? AND time=? AND status='pending'").get(date, time);
  ...
  const r = db.prepare(
    `INSERT INTO appointments (user_id,vehicle_id,name,email,phone,vehicle,oil_type,date,time,notes,location,is_recurring,recurrence_weeks)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(req.session.customerId, vehicle_id || null, name, email || '', phone, vehicle,
        oil_type || 'Convencional', date, time, notes || '', location,
        is_recurring ? 1 : 0, recurrence_weeks || 12);
  const appt = db.prepare('SELECT a.*, v.color as vehicle_color FROM appointments a LEFT JOIN vehicles v ON a.vehicle_id=v.id WHERE a.id=?').get(r.lastInsertRowid);
  sendConfirmation(appt);
  res.json({ success: true, id: r.lastInsertRowid });
});

app.delete('/api/my-appointments/:id', requireCustomer, (req, res) => {
  const appt = db.prepare('SELECT id FROM appointments WHERE id=? AND user_id=?').get(+req.params.id, req.session.customerId);
...
// ─── Admin API ────────────────────────────────────────────────────────────────
app.get('/api/admin/appointments/upcoming', requireAdmin, (_req, res) => {
  const today = new Date().toISOString().split('T')[0];
  const end = new Date(); end.setDate(end.getDate() + 14);
  res.json(
    db.prepare(`SELECT a.*, t.name as technician_name, v.color as vehicle_color FROM appointments a
                LEFT JOIN users t ON a.technician_id = t.id
                LEFT JOIN vehicles v ON a.vehicle_id = v.id
                WHERE a.date>=? AND a.date<=? AND a.status='pending' ORDER BY a.date,a.time`)
      .all(today, end.toISOString().split('T')[0])
  );
});

app.get('/api/admin/appointments', requireAdmin, (_req, res) => {
  res.json(
    db.prepare(`SELECT a.*, u.email as user_email, t.name as technician_name, v.color as vehicle_color FROM appointments a
                LEFT JOIN users u ON a.user_id = u.id
                LEFT JOIN users t ON a.technician_id = t.id
                LEFT JOIN vehicles v ON a.vehicle_id = v.id
                ORDER BY a.date DESC, a.time ASC`).all()
  );
});
...
app.get('/api/admin/stats', requireAdmin, (_req, res) => {
  ...
  res.json({
    today:    db.prepare("SELECT COUNT(*) c FROM appointments WHERE date=? AND status='pending'").get(today).c,
    week:     db.prepare("SELECT COUNT(*) c FROM appointments WHERE date>=? AND date<=? AND status='pending'").get(today, d7.toISOString().split('T')[0]).c,
    twoWeeks: db.prepare("SELECT COUNT(*) c FROM appointments WHERE date>=? AND date<=? AND status='pending'").get(today, d14.toISOString().split('T')[0]).c,
    pending:  db.prepare("SELECT COUNT(*) c FROM appointments WHERE status='pending'").get().c,
...
app.put('/api/admin/appointments/:id/complete', requireAdmin, (req, res) => {
  ...
  db.prepare(`UPDATE appointments SET status='completed', mileage=?,
    tire_fl=?, tire_fr=?, tire_rl=?, tire_rr=?, brake_front=?, brake_rear=? WHERE id=?`)
    .run(mileage ?? null, tire_fl ?? null, tire_fr ?? null, tire_rl ?? null, tire_rr ?? null,
         brake_front ?? null, brake_rear ?? null, appt.id);
  // Keep vehicle_health table in sync for backward compat
  const hasHealth = [tire_fl, tire_fr, tire_rl, tire_rr, brake_front, brake_rear].some(v => v != null);
  if (appt.user_id && hasHealth) {
    db.prepare(`INSERT INTO vehicle_health (user_id,vehicle,tire_fl,tire_fr,tire_rl,tire_rr,brake_front,brake_rear,updated_at,appointment_id)
      VALUES (?,?,?,?,?,?,?,?,datetime('now'),?)
      ...
      .run(appt.user_id, appt.vehicle, tire_fl??null,tire_fr??null,tire_rl??null,tire_rr??null,brake_front??null,brake_rear??null,appt.id);
  }
  ...
  if (appt.is_recurring) {
    ...
    const r = db.prepare(
      `INSERT INTO appointments (user_id,vehicle_id,name,email,phone,vehicle,oil_type,date,time,notes,location,is_recurring,recurrence_weeks,parent_id)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,1,?,?)`
    ).run(appt.user_id,appt.vehicle_id,appt.name,appt.email,appt.phone,appt.vehicle,appt.oil_type,
          nextDate,appt.time,appt.notes,appt.location||'',appt.recurrence_weeks,appt.id);
...
app.put('/api/admin/appointments/:id/assign', requireAdmin, (req, res) => {
  const { technician_id } = req.body;
  db.prepare('UPDATE appointments SET technician_id=? WHERE id=?').run(technician_id || null, +req.params.id);
  res.json({ success: true });
});
...
app.post('/api/admin/appointments', requireAdmin, (req, res) => {
  ...
  const r = db.prepare(
    `INSERT INTO appointments (user_id,vehicle_id,name,email,phone,vehicle,oil_type,date,time,notes,location,technician_id)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(user_id||null,vehicle_id||null,name,email||'',phone,vehicle,oil_type||'Convencional',
...
app.get('/api/admin/technicians', requireAdmin, (_req, res) => {
  res.json(db.prepare("SELECT id,name,email,phone,created_at FROM users WHERE role='technician' ORDER BY name").all());
});
app.post('/api/admin/technicians', requireAdmin, (req, res) => {
  ...
  const r = db.prepare("INSERT INTO users (name,email,phone,password_hash,role) VALUES (?,?,?,?,'technician')")
    .run(name.trim(), email.toLowerCase(), phone||'', hashPassword(password));
  res.json({ success: true, id: r.lastInsertRowid });
});
...
app.put('/api/admin/technicians/:id/password', requireAdmin, (req, res) => {
  const { password } = req.body;
  if (!password || password.length < 6) return res.status(400).json({ error: 'Mínimo 6 caracteres' });
  const tech = db.prepare("SELECT id FROM users WHERE id=? AND role='technician'").get(+req.params.id);
...
// ─── SendGrid config routes ───────────────────────────────────────────────────
app.get('/api/admin/settings/sendgrid', requireAdmin, (_req, res) => {
  const apiKeySet = !!(getSetting('sg_api_key') || process.env.SENDGRID_API_KEY);
  res.json({
    api_key_set: apiKeySet,
    from_email: getSetting('sg_from_email') || process.env.SENDGRID_FROM || '',
    from_name:  getSetting('sg_from_name')  || 'Cambio de Aceite',
    configured: sgConfigured,
  });
});

app.put('/api/admin/settings/sendgrid', requireAdmin, (req, res) => {
  ...
  if (api_key?.trim()) setSetting('sg_api_key', api_key.trim());
  setSetting('sg_from_email', from_email.trim());
  setSetting('sg_from_name', (from_name || 'Cambio de Aceite').trim());
  buildSendGrid();
  res.json({ success: true, configured: sgConfigured });
});

app.post('/api/admin/settings/sendgrid/test', requireAdmin, async (req, res) => {
  const to = getSetting('sg_from_email') || process.env.SENDGRID_FROM || '';
  ...
  const ok = await sendEmail(to, 'appointment_confirmed', {
    ...
    vehicle: 'Toyota Camry 2022', oil_type: 'Sintético — $85',
    location_row: '', notes_row: '',
  });
...
app.get('/api/admin/email-templates', requireAdmin, (_req, res) => {
  ...
  TEMPLATE_TYPES.forEach(type => {
    result[type] = {
      subject: getSetting(`tpl_${type}_subject`) || DEFAULT_TEMPLATES[type].subject,
      body:    getSetting(`tpl_${type}_body`)    || DEFAULT_TEMPLATES[type].body,
      is_custom: !!(getSetting(`tpl_${type}_subject`) || getSetting(`tpl_${type}_body`)),
    };
  });
  res.json(result);
});
...
app.put('/api/admin/email-templates/:type', requireAdmin, (req, res) => {
  ...
  setSetting(`tpl_${type}_subject`, subject.trim());
  setSetting(`tpl_${type}_body`, body.trim());
  res.json({ success: true });
});

app.delete('/api/admin/email-templates/:type', requireAdmin, (req, res) => {
  const { type } = req.params;
  if (!TEMPLATE_TYPES.includes(type)) return res.status(400).json({ error: 'Tipo inválido' });
  db.prepare("DELETE FROM settings WHERE key=?").run(`tpl_${type}_subject`);
  db.prepare("DELETE FROM settings WHERE key=?").run(`tpl_${type}_body`);
  res.json({ success: true, default: DEFAULT_TEMPLATES[type] });
});

app.get('/api/admin/customers', requireAdmin, (_req, res) => {
  res.json(db.prepare("SELECT id,name,email,phone FROM users WHERE role='customer' ORDER BY name").all());
});

// ─── Technician API ───────────────────────────────────────────────────────────
app.get('/api/tech/me', requireTech, (req, res) => {
  const user = db.prepare('SELECT id,name,email,phone FROM users WHERE id=?').get(req.session.customerId);
  res.json(user || {});
});
app.get('/api/tech/appointments', requireTech, (req, res) => {
  const today = new Date().toISOString().split('T')[0];
  res.json(
    db.prepare(`SELECT a.*, v.color as vehicle_color FROM appointments a
                LEFT JOIN vehicles v ON a.vehicle_id = v.id
                WHERE a.technician_id=? AND a.date>=? AND a.status='pending' ORDER BY a.date,a.time`)
      .all(req.session.customerId, today)
  );
});
app.put('/api/tech/appointments/:id/complete', requireTech, (req, res) => {
  const appt = db.prepare('SELECT * FROM appointments WHERE id=? AND technician_id=?').get(+req.params.id, req.session.customerId);
  if (!appt) return res.status(404).json({ error: 'Not found' });
  if (appt.status === 'completed') return res.status(409).json({ error: 'Ya completada' });
  const { tire_fl, tire_fr, tire_rl, tire_rr, brake_front, brake_rear, mileage } = req.body || {};
  db.prepare(`UPDATE appointments SET status='completed', mileage=?,
    tire_fl=?, tire_fr=?, tire_rl=?, tire_rr=?, brake_front=?, brake_rear=? WHERE id=?`)
    .run(mileage??null,tire_fl??null,tire_fr??null,tire_rl??null,tire_rr??null,brake_front??null,brake_rear??null,appt.id);
  res.json({ success: true });
});
...
app.post('/auth/forgot-password', async (req, res) => {
  ...
  const user = db.prepare("SELECT id, name, email FROM users WHERE email=? AND role='customer'").get(email.trim().toLowerCase());
...
function getPayPalBase() {
  return (getSetting('paypal_mode') || 'sandbox') === 'live' ? PAYPAL_LIVE : PAYPAL_SANDBOX;
}

async function getPayPalToken() {
  const clientId = getSetting('paypal_client_id');
  const secret   = getSetting('paypal_client_secret');
...
// Public: solo expone client_id (sin secreto)
app.get('/api/settings/paypal-client', (_req, res) => {
  const clientId = getSetting('paypal_client_id');
  res.json({ client_id: clientId || '', mode: getSetting('paypal_mode') || 'sandbox', enabled: !!clientId });
});
...
app.post('/api/payments/create-order', requireCustomer, async (req, res) => {
  ...
  try {
    const r = await fetch(`${getPayPalBase()}/v2/checkout/orders`, {
      ...
      body: JSON.stringify({
        intent: 'CAPTURE',
        purchase_units: [{ amount: { currency_code: 'USD', value: price }, description: `Cambio de aceite — ${oil_type}` }]
      })
    });
    const d = await r.json();
...
// Capturar orden PayPal + crear cita
app.post('/api/payments/capture-order', requireCustomer, async (req, res) => {
  ...
  if (!token) return res.status(503).json({ error: 'PayPal no configurado' });
  try {
    const captureRes = await fetch(`${getPayPalBase()}/v2/checkout/orders/${order_id}/capture`, {
      method: 'POST',
...
// Admin — configuración PayPal
app.get('/api/admin/settings/paypal', requireAdmin, (_req, res) => {
  res.json({
    client_id: getSetting('paypal_client_id') || '',
    client_secret_set: !!getSetting('paypal_client_secret'),
    mode: getSetting('paypal_mode') || 'sandbox',
    configured: !!(getSetting('paypal_client_id') && getSetting('paypal_client_secret')),
  });
});

app.put('/api/admin/settings/paypal', requireAdmin, (req, res) => {
  const { client_id, client_secret, mode } = req.body;
  if (!client_id?.trim()) return res.status(400).json({ error: 'Client ID es requerido' });
  setSetting('paypal_client_id', client_id.trim());
  if (client_secret?.trim()) setSetting('paypal_client_secret', client_secret.trim());
  setSetting('paypal_mode', mode === 'live' ? 'live' : 'sandbox');
  res.json({ success: true });
});
